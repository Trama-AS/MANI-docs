...
| PROY-08 | El proyecto exige el uso de **Kubernetes** como orquestador — requisito del profesor (curricular), no una decisión evaluable por atributo de calidad. Cierra la pregunta de "¿se justifica Kubernetes?" de la Matriz de Herramientas (incompatibilidad #4). Kubernetes no tiene costo de licencia (ver SRS_MANI.md §1.4); el costo antes atribuido a "Kubernetes" era en realidad de Azure AKS y ya no aplica (Azure retirado, ADR-0021). Sigue abierto el **cómo** (distribución/config concreta del clúster, sin proveedor ratificado) — eso sí requiere ADR | **Crítica** |
