# TRAMA · MANI — Documento de Herramientas, Políticas y Lineamientos V3

**Proyecto:** MANI — Plataforma Multi-Tenant de Formalización y Gestión de Servicios  
**Organización:** TRAMA · Ingeniería de Software  
**Documento:** Herramientas, Políticas y Lineamientos V3 (Entrega 4 — Sprint 2 Review & Planning Sprint 3)  
**Versión:** 3.0 (Borrador de línea base para Sprint 3 — construido sobre V2.1; lo agregado o modificado se marca con 🆕)  
**Fecha:** 2026-09-23  
...
**Fuentes y Trazabilidad:** ADR-0001 a ADR-0026 (Repositorio `Trama-AS/MANI-docs`), SAD V3, SRS V3, SDD V1, Documento de Infraestructura V1, DOC-08, DOC-14, PoC-001..004, Inf_test-002, Gobierno del Equipo.
...
| **2.1** | 2026-09-02 | Daniel Ávila, Juan Sebastián Álvarez, Santiago, Mesa de Arquitectura | Actualización mayor: Incorporación del nuevo **Diagrama de Tecnologías de Alto Nivel V2** (estandarización de almacenamiento y compilación Docker), integración de **ADR-0014** (Patrón Feature Toggle para desacoplamiento de despliegue y control por tenant) y **ADR-0018** (Identificación criptográfica de tenant mediante JWT claims anti-spoofing y token relay), eliminación de diagramas ASCII en favor de diagramas formales en **Mermaid**, y reubicación centralizada de activos visuales en `/Diagramas/Tecnologías/`. | Superada |
| **3.0** 🆕 | 2026-09-23 | Equipo TRAMA (borrador para Mesa) | Se conserva todo V2.1 y se añade: retiro de **Azure** y consolidación en **Railway + GHCR** (ADR-0023); Kubernetes sin AKS (propuesta k3d/kind, ADR-0029 borrador); workflows reales por rama; **SonarCloud** solo en `main` y ZAP pendiente; nuevas herramientas y técnicas adoptadas por las PoC (**k6**, **Newman con control negativo**, `database/migrations` + `database/verify`, **Structurizr DSL**, **Maestro**); Tech Radar V3 (Azure → Mejor no, Railway → Sí o sí, Spring Boot condicionado); **DoR/DoD** de ADR-0025/0026; política de **PoC y spikes**; política de **migraciones de base de datos**; estado de cumplimiento verificado (§7). | Borrador |
...
> 🆕 V3: 🔴 la imagen `Diagramas/Tecnologías/DiagramaTecnologiasAltoNivel_V2.jpg` no está en el repositorio (la carpeta `Tecnologías/` no existe). Debe subirse o regenerarse sin Azure. La vista equivalente actualizada es el diagrama de despliegue del Documento de Infraestructura V1 §3.
...
- **Repo C (`mani-backend-dotnet`):** Microservicio backend implementado en **.NET (C#)**. Automatiza su compilación, pruebas y publicación hacia Azure Container Docker Registry (ACR).
  - 🆕 **V3:** Azure retirado (ADR-0023). Registro propuesto: **GHCR** para los tres repos (ya operativo en `MANI-Flutter`; Infraestructura V1 M-02).
- 🆕 **Backend Core (Serverpod/Dart):** dueño del dominio y del esquema (ADR-0012). Alcance propuesto para Java = Motor de Reglas y .NET = Motor de Despacho, más un **API Gateway** Spring Cloud Gateway (SDD V1, ADR-0027/0028 borradores).
...
- 🆕 **Estado real V3 (`MANI-Flutter`):** `ci.yml` por rama (`develop`, `release`, `main`) con el job **`Flutter Lint, Test & Build Check`** (format → analyze → test con cobertura → integración → build web); en `release` además aplica migraciones a Supabase QA; `staging-artifact.yml` empaqueta `flutter-web-staging.zip`; en `main` corre SonarCloud + Quality Gate; `docker-publish.yml` publica en GHCR (`latest`, semver) y `release.yml` crea el GitHub Release por tag.
...
> 🆕 **V3 — reemplaza el texto anterior de §1.4 en lo relativo al proveedor:** Microsoft Azure se retiró (ADR-0023). Los ambientes son los de **DOC-08**: DEV en Docker Compose local, QA en Supabase Cloud (proyecto QA) + Railway Staging, PROD en Supabase Cloud (proyecto PROD) + Railway Production. **Kubernetes sin AKS:** clúster k3d/kind de referencia con las mismas imágenes y manifiestos Kustomize (ADR-0029 borrador; Documento de Infraestructura V1 §9). La rama de QA es `release` persistente (desvío documentado en Inf_test-002).
...
![Tech Radar V2 del Proyecto MANI](../../Diagramas/TECH_RADARv2.png)

> 🆕 V3: ruta corregida — el archivo vive en `Diagramas/TECH_RADARv2.png`. Los cambios de anillo de V3 están en §2.3; falta regenerar la imagen como `TECH_RADARv3.png`.
...
### 🆕 2.3 Cambios del Tech Radar V3 (sobre la tabla V2, que se conserva arriba)

| Tecnología | Anillo V2 | Anillo V3 propuesto | Motivo | Fuente |
| :--- | :---: | :---: | :--- | :--- |
| **Microsoft Azure** | Tal vez | **MEJOR NO** | Retirado como proveedor por costo fijo | ADR-0023 |
| **Kubernetes (AKS)** | Sí o sí (AKS) | **Sí o sí — sin AKS** | Orquestador obligatorio (PROY-08); clúster k3d/kind de referencia | ADR-0010, ADR-0029 (borrador) |
| **Railway** | Tal vez | **SÍ O SÍ** | Hosting de QA/PROD sin costo fijo | ADR-0023 |
| **GHCR (GitHub Container Registry)** | — | **SÍ O SÍ** | Registro operativo en CI; propuesta de unificar (Docker Hub queda en Tal vez) | Infraestructura V1 M-02 |
| **Supabase Auth** | (dentro de BaaS) | **SÍ O SÍ** | IdP ratificado; claims por hook validados | ADR-0022, PoC-002 |
| **Supabase Storage** | (dentro de BaaS) | **SÍ O SÍ** | Bucket KYC validado | ADR-0013, PoC-003 |
| **Supabase Realtime** | (dentro de BaaS) | TAL VEZ | ADR-0017 Propuesto, sin PoC | SP-TO-06 |
| **Spring Boot / Spring Cloud Gateway** | Mejor no | **TAL VEZ → Sí o sí si se ratifican ADR-0027/0028** | Gateway con validación JWKS sin licencia; Motor de Reglas; límite de memoria explícito (`-XX:MaxRAMPercentage`) | SDD V1 §5.3 |
| **ASP.NET Core** | (dentro de .NET) | SÍ O SÍ | Motor de Despacho | ADR-0028 (borrador) |
| **SonarCloud** | (SonarQube) | **SÍ O SÍ en `main`** | Único punto donde corre hoy (403 del Quality Gate fuera de `main` desde el 20/09) | Inf_test-002 §2.4 |
| **OWASP ZAP** | Tal vez | Tal vez | Aún no integrado en ningún workflow | Infraestructura V1 S-07 |
| **k6** | Sí o sí | Sí o sí — **usado** | PoC-001, PoC-004 | Inf_PoC-001 |
| **Newman** | Sí o sí | Sí o sí — **usado** | Suites `mani-claims`, `mani-aislamiento` (135/135) | Inf_test-002 |
| **Maestro** | — | **TAL VEZ** | Pruebas E2E de UI Flutter exigidas por el DoD propuesto | ADR-0025 (Propuesto) |
| **Structurizr DSL** | — | **SÍ O SÍ** | Fuente editable única del modelo C4 | ADR-0024, SDD V1 §3.3 |
| **Mermaid** | Sí o sí (técnica) | Sí o sí | Secuencias UML versionadas | ADR-0008 |
| **Draw.io / Miro** | — | SÍ O SÍ | Diagramación técnica formal / BPMN | ADR-0020, ADR-0024 |
| **PostGIS** | — | TAL VEZ | Solo si se activa ADR-0011 §6 (100 % de precisión, 7,17 ms) | PoC-004 |
| **Geohash** | — | **MEJOR NO** | 93,78 % de precisión en bordes | PoC-004 |
| **Prometheus / Grafana / Datadog** | Sí o sí | **Sí o sí — sin instrumentar** | Decididos en ADR-0006, sin servicios que instrumentar todavía; Gobierno §2.1 los marca como pendientes de ratificar | ADR-0006 |
...
### 🆕 3.1 Actualización V3 de la matriz

| Herramienta | Naturaleza | Etapa | Propósito en MANI | Responsable | Estado V3 |
| :--- | :--- | :--- | :--- | :--- | :---: |
| **Kubernetes (AKS)** → **Kubernetes (k3d/kind)** | Producto | Operate | Clúster de referencia con las mismas imágenes (PROY-08) | Daniel Ávila | **Propuesta (ADR-0029)** |
| **Railway** | Producto | Deploy | Hosting de backends y gateway en QA/PROD | Daniel Ávila | **Adoptada (ADR-0023), sin despliegues aún** |
| **GHCR** | Proceso | Package | Registro de imágenes (`latest`, semver) | Daniel Ávila | **Operativa** |
| **SonarCloud** | Proceso | Security (SAST) | Quality Gate en `main` | Daniel Ávila / Santiago | **Operativa solo en `main`** |
| **OWASP ZAP** | Proceso | Security (DAST) | Baseline sobre QA tras cada promoción | Santiago | **Pendiente** |
| **k6** | Proceso | Test (Perf) | Concurrencia de despacho (PoC-001), latencia de cobertura (PoC-004) | Santiago | **Operativa** (`qa/k6/`) |
| **Newman** | Proceso | Test | `mani-claims` (40), `mani-aislamiento` (95 con caso 6) | Santiago | **Operativa manual** (`qa/newman/`) |
| **Harness de Storage** | Proceso | Test | Carga KYC y URLs firmadas (PoC-003) | Santiago | **Operativa** (`qa/storage/`) |
| **`database/migrations` + `database/verify`** | Proceso / Producto | Data | Cadena idempotente 001–007 aplicada por CI a QA; verificaciones de solo lectura | Backend Lead / Santiago | **Operativa** |
| **Docker Compose + Adminer** | Proceso | Dev | DEV local (`mani-web`, `postgres:16-alpine`, Adminer) | Daniel Ávila | **Operativa** |
| **Structurizr DSL** | Proceso | Design | `Diagramas/c4/workspace-as-built.dsl` | Mesa de Arquitectura | **Adoptada (ADR-0024)** |
| **Miro** | Proceso | Design | BPMN de negocio | Nicolás León | **Adoptada (ADR-0024)** |
| **Maestro** | Proceso | Test (E2E UI) | Regresión de flujos Flutter | QA | **Propuesta (ADR-0025)** |
| **Spring Cloud Gateway** | Producto | Code | API Gateway | Backend | **Propuesta (ADR-0027)** |
| **Serverpod** | Producto | Code | Backend Core | Juan Sebastián Álvarez | **Adoptada, sin código** |
...
4. 🆕 **V3 — correspondencia real:** DEV = `.env` local (fuera de Git); QA = GitHub Repository Secrets (`SUPABASE_QA_DB_URL` configurado el 2026-09-23) + variables de Railway; PROD = **GitHub Environment `production` con revisor obligatorio (DevOps titular)** — ⬜ aún no creado; K8s = `Secret` creados desde el pipeline. La `service_role` key de Supabase solo existe en el entorno del Core. Inventario completo en Documento de Infraestructura V1 §8.
5. 🆕 **Gates verificados en la última promoción (Inf_test-002):** DEV→QA cumplió CI (313 pruebas, cobertura 85,4 %) pero **no** cumplió revisión por pares ni DoR (SCRUM-1055, SCRUM-1056); se promovió con excepción documentada.
...
> 🆕 **V3:** la DoR y la DoD de esta sección se conservan como referencia de Sprint 2. Para Sprint 3 se proponen las versiones de **ADR-0026 (DoR para paso a QA)** y **ADR-0025 (DoD para pases a `main`)**, detalladas en §4.9.
...
## 🆕 4.9 DoR y DoD V3 (ADR-0026 y ADR-0025, Propuestos)

**DoR — paso a QA (ADR-0026).** Además de la DoR de Sprint 2, el desarrollador adjunta en
Jira antes de mover a QA:
- [ ] Criterios de aceptación Gherkin aceptados por el PO; por cada criterio, un caso positivo, uno negativo y un intento de violar la regla crítica.
- [ ] Evidencia funcional verificable (captura o video) del flujo en la app.
- [ ] Pruebas automatizadas trazables a cada criterio (nombre del test ↔ criterio).
- [ ] Comprobación de aislamiento multi-tenant con al menos dos tenants locales.
- [ ] Enlace al run de CI en verde.

**DoD — pase a `main` (ADR-0025).**
- [ ] Pipelines en verde en `release` y en el PR hacia `main`.
- [ ] Suites según la arquitectura afectada: Newman (`mani-aislamiento`, `mani-claims`) si toca datos/auth/RLS; Maestro si toca UI; k6 si toca despacho o búsqueda.
- [ ] Regresión manual enfocada en dependencias del cambio.
- [ ] Quality Gate de SonarCloud aprobado.
- [ ] Aceptación del PO y merge ejecutado por el DevOps titular.

## 🆕 4.10 Política de PoC y spikes

Aprendida de PoC-001..004 (Inf_PoC-001) y `Entregas/Spikes/README.md`:
1. **PoC ≠ spike.** Un spike responde una pregunta abierta antes de decidir; una PoC verifica una decisión ya redactada en un ADR.
2. **Umbral publicado antes de medir**, en el ticket de Jira.
3. **Control negativo obligatorio**: una variante que debe fallar. Si no puede fallar, no valida nada (PoC-001 H-01; PoC-002: con el hook apagado, 27/40 aserciones seguían pasando).
4. Cada caso negativo de aislamiento lleva su **espejo positivo**.
5. **Evidencia cruda versionada** (bitácora SQL, JSON de k6/Newman **redactados**), reproducible con un README.
6. Credenciales provisionadas **antes** de la ventana medida (Supabase Auth responde `429` hacia el login 30 en 5 min por IP).
7. La concurrencia efectiva la acota el pool de PostgREST, no `max_connections`.
8. Conclusión explícita para el ADR: confirmar, corregir o reabrir.
9. **Terminado** = informe mergeado en `MANI-docs` (evita el desfase Jira ↔ repo visto con PoC-002/003).

## 🆕 4.11 Política de migraciones de base de datos

1. Ningún cambio de esquema a mano en Supabase (*schema-first*, DOC-08 §5.1).
2. Migraciones numeradas `NNN_descripcion.sql` en `database/migrations/`, **idempotentes** (se prueban dos veces seguidas).
3. Flujo: Git → Docker DEV → Supabase QA (CI de `release`) → Supabase PROD (CI de `main` con aprobación de Environment).
4. Toda política RLS vive en la cadena de migraciones. 🔴 Hoy las 16 `tenant_isolation_*` solo existen en QA (SCRUM-1051): prohibido aprovisionar PROD hasta versionarlas.
5. Cada migración que cambia datos trae un script de verificación de solo lectura en `database/verify/`.
6. Valores de dominio en MAYÚSCULA en español en la BD; rol en minúscula en el JWT (migración 007).

## 🆕 4.12 Política de documentación de arquitectura (ADR-0024)

- Vistas organizadas según **4+1**; notación según ADR-0024 (C4, UML, BPMN, MER) — SDD V1 §3.
- **Fuente editable única por tipo de diagrama**; un diagrama exportado sin fuente en el repositorio no se considera vigente.
- Todo ADR nuevo toma el siguiente número libre de la carpeta `ADR/` (los borradores del SDD pasaron de 0025/0026 a 0027/0028 por colisión).
...
> 🆕 V3: esta tabla corresponde a la aprobación de la **V2.1**. La aprobación de V3 está en §8.
...
# 🆕 7. Estado de cumplimiento verificado (2026-09-23)

"Lo que está aquí, se cumple" (§0). Esta tabla registra dónde la práctica aún no alcanza a la política, para decidir en retrospectiva si se corrige la práctica o la política.

| Política | Estado | Evidencia | Acción |
| :--- | :---: | :--- | :--- |
| §4.1 Sin commits directos a `develop` | ❌ | 8 commits directos (Inf_test-002) | Rulesets en `develop`/`release` (SCRUM-1056) |
| §4.1 PR con una aprobación | ❌ | #9 y #14 sin revisión | Regla `pull_request` en rulesets |
| §4.1 Quality Gate en todo PR | ◐ | Sonar solo en `main` | Evaluar Sonar en PR a `release` |
| §4.2 Environment Protection Rules | ❌ | No hay environments configurados | Crear `release` y `production` |
| §4.3.2 DAST con ZAP | ❌ | No integrado | Job `zap-baseline` en `release` |
| §4.3.3 Suite de 6 casos en cada PR | ◐ | Suite existe y pasa (135/135); no corre en PR | Job Newman en el workflow |
| §4.3.4 Tenant en JWT | ◐ | Hook validado en PoC-002; aún no en migraciones | Migración 010 |
| §4.3.5 KYC aislado | ✅ | PoC-003 95/95 | Corregir H-01/H-02/H-04 en ADR-0013 |
| §4.4 Logs JSON con `trace_id`/`tenant_id` | ◐ | `StructuredLogger` en la app; sin backends | — |
| §4.7 DoR con criterios BDD | ❌ | 8 de 9 historias sin criterios | Backlog V3 §3 propone criterios (SCRUM-1055) |
| §4.11 RLS versionado | ❌ | SCRUM-1051 | Migración 008 |

# 🆕 8. Aprobación V3

| Rol | Integrante | Estado |
| :--- | :--- | :---: |
| Product Owner | Nicolás León | Pendiente |
| Scrum Master | Sara Albarracín | Pendiente |
| DevOps Titular | Daniel Ávila | Pendiente |
| QA Lead / Security | Santiago | Pendiente |
| Frontend Leads | Camila Beltrán / Nicolás Álvarez | Pendiente |
| Backend Lead | Juan Sebastián Álvarez | Pendiente |
| Mesa de Arquitectura | Quórum 5 de 7 | Pendiente de ratificación (§0, regla 3) |
